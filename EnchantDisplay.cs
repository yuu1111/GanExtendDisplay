...
namespace GanExtendDisplay
{
    internal static class EnchantDisplay
    {
            ...
            Action<UINote, Element> onAddNote = null)
        {
            ...
            switch (mode)
            {
                ...
                case ElementContainer.NoteMode.BonusTrait:
                {
                    ...
                    else
                    {
                        ...
                        flag2 = !(flag6 || flag7);
                        text = (flag6 ? "textEncSkill" : (flag7 ? "textEncEnc" : "textEnc")).lang(name, num + (__instance.source.tag.Contains("ratio") ? "%" : ""),
                            ((__instance.Value > 0) ? "encIncrease" : "encDecrease").lang());
                    }
                    ...
                    {
                        text = text + " [" + "*".Repeat(Mathf.Clamp(num * __instance.source.mtp / num4 + num3, 1, 5)) + ((num * __instance.source.mtp / num4 + num3 > 5) ? "+" : "") + "]" +
                               $"({num})";
                    }
            ...
            UIItem uIItem2 = n.AddTopic("TopicAttribute", __instance.Name,
                "".TagColor((__instance.ValueWithoutLink > 0) ? SkinManager.CurrentColors.textGood : SkinManager.CurrentColors.textBad, __instance.ValueWithoutLink.ToString()));
